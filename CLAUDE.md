...
- `flight_agent` → `tools/flight_tool.py::search_flights` (AviationStack API)
- `hotel_agent`  → `tools/tavily_tool.py::tavily_search` (Tavily Search API)
- `itinerary_agent` / `final_agent` → Groq LLM (`llama-3.3-70b-versatile`)
- State: `TravelState` (TypedDict) in `main.py`
- Memory: `PostgresSaver` keyed by `thread_id`
...
| `frontend.py` | Streamlit web UI (streams agent steps live) |
| `tools/flight_tool.py` | AviationStack flight search |
| `tools/tavily_tool.py` | Tavily hotel/web search |
| `mcp_server/travel_mcp.py` | MCP server exposing the travel tools |
...
- `PostgresSaver.setup()` requires an **autocommit** connection (see `main.py`).
- `frontend.py` imports `app` from `main.py`, which builds the graph at import
  time (and runs `setup()`). Be careful changing import-time side effects.
- Known issues and the path to production are documented in
  `PRODUCTION_READINESS.md` — consult it before large changes. Notably:
  `search_flights` currently ignores the user query, and there is a single
  global DB connection (no pool).
